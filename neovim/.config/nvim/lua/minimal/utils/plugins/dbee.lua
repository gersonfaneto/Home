...
local file_name = nui.input({
  ...
  on_submit = function(value)
    ...
    if range then
      ...
      require("dbee").store(format, op_type, { extra_arg = op_type == "file" and join_paths(save_path, filename) or 0 })
    end
    vim.notify("Saved: " .. filename .. " to: " .. save_path, vim.log.levels.INFO, { title = "Dbee" })
  end,
})

local range_input = nui.input({
  position = {
    col = "50%",
    row = "30%",
  },
...
query:on(nui.event.BufLeave, function()
  query:unmount()
end)

-- unmount component when cursor leaves buffer
file_name:on(nui.event.BufLeave, function()
  file_name:unmount()
end)

type_chooser:on(nui.event.BufLeave, function()
  type_chooser:unmount()
end)

format_chooser:on(nui.event.BufLeave, function()
